...
@implementation BaseAuth

@synthesize bridge = _bridge;
RCT_EXPORT_MODULE(BaseAuth);
...
RCT_EXPORT_METHOD(getDataWithKey:(NSDictionary *)options withKey:(NSString *)key resolver:(RCTPromiseResolveBlock)resolve rejecter:(RCTPromiseRejectBlock)reject) {
//  NSString *service = serviceValue(options);
  NSString *accessGroup = accessGroupValue(options);
...
//  queryParts[(__bridge NSString *)kSecAttrService] = service;
  queryParts[(__bridge NSString *)kSecReturnAttributes] = (__bridge id)kCFBooleanTrue;
  queryParts[(__bridge NSString *)kSecUseOperationPrompt] = authenticationPrompt;
  
  if (accessGroup != nil) {
    queryParts[(__bridge NSString *)kSecAttrAccessGroup] = accessGroup;
  }
  
//  queryParts[(__bridge NSString *)kSecAttrGeneric] = key;
  queryParts[(__bridge NSString *)kSecAttrAccount] = key;
  queryParts[(__bridge NSString *)kSecReturnData] = (__bridge id)kCFBooleanTrue;
